...
f = open(filename, "w")
headers = "Name,Friend Count,Photo Count,Review Count,Elite Member,Funny Count,Cool Count,Useful Count,Review Length,Checkin Count,Review_Body\n"  # these are the features that are scraped
f.write(headers)

for _ in range(4):  # regex could have been used here but this is to increment the url page(keeping it simple.)
    my_url = "https://www.yelp.com/biz/musashis-seattle?&start=" + str(x)
   ...
   #  print("Amount of reviews on this page" + len(container))
    ...
    for i in container:
        ...
        f.write(
            str(user_count) + "," + str(friend_count) + "," + str(photo_count) + "," + str(review_count) + "," + str(
                elite_count) + "," + str(funny_count) + "," + str(cool_count) + "," + str(useful_count) + "," + str(
                length_count) + "," + str(checkin_count) + "," + str(review) + "\n")
